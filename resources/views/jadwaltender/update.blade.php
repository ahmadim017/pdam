            ...
            <div class="table-responsive">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <input type="hidden" name="jadwal[{{$loop->index}}][id_paket]" value="{{ $jadwal->id_paket ?? ''}}">
                        <input type="hidden" name="jadwal[{{$loop->index}}][kegiatan]" value="{{ $jadwal->kegiatan ?? ''}}">
                        <tr>
                            <td>{{$loop->iteration}}</td>
                            <td>{{$jadwal->kegiatan}}</td>
                            <td><input type="date" name="jadwal[{{$loop->index}}][tglpelaksanaan]" value="{{ old('tglpelaksanaan', $jadwal->tglpelaksanaan ?? '') }}" class="form-control"></td>
                            <td>
                                <div class="input-group">
                                    <input type="time" class="form-control" name="jadwal[{{$loop->index}}][waktumulai]"  value="{{ old('waktumulai', $jadwal->waktumulai ?? '') }}">
                                    <span class="input-group-text">-</span>
                                    <input type="time" class="form-control" name="jadwal[{{$loop->index}}][waktuselesai]" value="{{ old('waktuselesai', $jadwal->waktuselesai ?? '') }}">
                                </div>
                            </td>
                        </tr>
                ...
                <a href="{{route('tender.show',[$pengadaan->id])}}" class="btn btn-secondary btn-sm"><i class="fa fa-arrow-circle-left fa-fw fa-sm"></i> Kembali</a>
                </form>
