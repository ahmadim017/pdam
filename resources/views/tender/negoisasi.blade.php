  ...
  <div class="col">
    <nav aria-label="breadcrumb" class="bg-light rounded-3 p-3 mb-4">
      <ol class="breadcrumb mb-0">
        ...
        <li class="breadcrumb-item text-secondary" aria-current="page"><a href="{{route('tender.pengumumanpemenang',[$pengadaan->id])}}">Pengumuman Pemenang</a></li>
...
<div class="col-md-12">
  <div class="card shadow mb-4">
      ...
      <a href="#collapseCardExample" class="d-block card-header py-3" data-toggle="collapse" role="button" aria-expanded="true" aria-controls="collapseCardExample">
        <h6 class="m-0 font-weight-bold text-primary">Negoisasi Teknis dan Biaya</h6>
      </a>
      @if(session('status'))
...
<form id="formnegoisasi" action="{{route('detailtender.banegoisasi',[$pengadaan->id])}}" method="POST">
@csrf


<tr>
  <th class="bg-light">Berita Acara Negoisasi</th>
  <td><strong><input type="text" name="banegoisasi" value="{{ old('banegoisasi', $detailtender->banegoisasi ?? '') }}" class="form-control {{$errors->first('banegoisasi') ? "is-invalid" : ""}}"></strong></td>
  <div class="invalid-feedbeck">
            <span class="text-danger">{{$errors->first('banegoisasi')}}</span>
          </div></td>
</tr>
<tr>
  <th class="bg-light">Tanggal Berita Acara</th>
  <td><strong><input type="date" name="tglnegoisasi" value="{{ old('tglnegoisasi', $detailtender->tglnegoisasi ?? '') }}" class="form-control {{$errors->first('tglnegoisasi') ? "is-invalid" : ""}}"></strong></td>
  <div class="invalid-feedbeck">
            <span class="text-danger">{{$errors->first('tglnegoisasi')}}</span>
          </div>
</tr>

<tr>
    <th class="bg-light">Hasil Negoisasi</th>
    <td><strong><input type="text" id="hasilnegoisasi" name="hasilnegoisasi" value="{{ old('hasilnegoisasi', $detailtender->hasilnegoisasi ?? '') }}" class="form-control {{$errors->first('hasilnegoisasi') ? "is-invalid" : ""}}"></strong></td>
    <div class="invalid-feedbeck">
              <span class="text-danger">{{$errors->first('hasilnegoisasi')}}</span>
            </div>
  </tr>
</form>
@if(optional($detailtender)->banegoisasi)
<tr>
  <th class="bg-light">Dokumen BA Negoisasi</th>
  <td>
   
    <a href="{{route('tender.banegoisasi',[$pengadaan->id])}}" target="_blank" class="btn btn-success btn-sm mr-2">Dokumen BA Negoisasi</a>
